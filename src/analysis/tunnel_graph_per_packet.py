    ...
    def parse_tunnel_log(self):
        ...
        while True:
            ...
            if event_type == '#':
                ...
                if flow_id not in self.ingress_tput:
                ...
                self.ingress_tput[flow_id].append(
                    (num_bits / 1000) / (ts - ingress_latest_ts))
                self.ingress_t[flow_id].append((ts - first_ts) / 1000.0)
                ingress_latest_ts = ts

                arrivals[flow_id] += num_bits
                ...
                if flow_id not in self.egress_tput:
                ...
                self.egress_tput[flow_id].append(
                    (num_bits / 1000) / (ts - egress_latest_ts))
                self.egress_t[flow_id].append((ts - first_ts) / 1000.0)
                egress_latest_ts = ts

                departures[flow_id] += num_bits
        ...
        for flow_id in self.flows:
            ...
            if flow_id in arrivals:
                ...
                if last_arrival_ts == first_arrival_ts:
                    ...
                    self.avg_ingress[flow_id] = arrivals[flow_id] / delta
            ...
            if flow_id in departures:
                ...
                if last_departure_ts == first_departure_ts:
                    ...
                    self.avg_egress[flow_id] = departures[flow_id] / delta
            ...
            if flow_id in arrivals and flow_id in departures:
                ...
                if arrivals[flow_id] > 0:
                    self.loss_rate[flow_id] = (
                        1 - 1.0 * departures[flow_id] / arrivals[flow_id])
